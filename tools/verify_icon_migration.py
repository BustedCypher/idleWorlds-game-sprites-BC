...
def verify(root:Path):
 root=root.resolve();h=(root/'index.html').read_text();mm=re.search(r"manifestUrl:\s*'(\./assets/icons/v2/icon-manifest\.[0-9a-f]{12}\.json)'",h);mp=root/mm.group(1).removeprefix('./');m=json.loads(mp.read_text());a=json.loads((mp.parent/m['audit_path']).read_text())
 ...
 ok=len(items)==4349 and len(gear)==3182 and len(other)==1167 and len(a['gear_records'])==1146 and len(a['item_cells'])==375 and ups=={'1':510,'2':510,'3':510,'4':510} and len(m['chunks'])==17 and not(gm or im or static or errors or mismatches or unhashed) and all(checks.values())
 return {'status':'ok' if ok else 'failed','items':len(items),'gear':len(gear),'other':len(other),'gear_cells':len(a['gear_records']),'item_cells':len(a['item_cells']),'upgrades':ups,'chunks':len(m['chunks']),'gear_missing':gm,'item_missing':im,'static_missing':static,'chunk_errors':errors,'pixel_mismatches':mismatches,'unhashed':unhashed,'checks':checks}
