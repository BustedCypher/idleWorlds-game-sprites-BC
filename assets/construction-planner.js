(function (root, factory) {
  ...
  function install(options) {
    ...
    function housingArt(tier) {
      var fallback='<span data-ui=cv-house-fallback class="cv-house-fallback" aria-hidden="true">'+o.houseArt(tier)+'</span>';
      if (!HOUSING_SPRITES[tier]) return '<span data-ui=cv-house-art class="cv-house-art cv-house-art-fallback-only">'+fallback+'</span>';
      return '<span data-ui=cv-house-art class="cv-house-art"><img class="cv-house-sprite" src="'+HOUSING_SPRITES[tier]+'" alt="" decoding="async">'+fallback+'</span>';
    }
    ...
    host.innerHTML = `
      <div data-ui=pane-head class="pane-head cv-page-head"><div data-ui=pane-content><div data-ui=pane-eyebrow class="pane-eyebrow">Skill Tools · Construction</div><h2 class="pane-title">Construction & Village Planner</h2><p class="pane-sub">Shape your village. Compare the benefits. Plan every material.</p></div><a class="cv-wiki" href="https://idleworlds.com/wiki/construction" target="_blank" rel="noopener noreferrer">Construction guide ↗</a></div>
      <div data-ui=cv-setup class="cv-setup panel"><div data-ui=cv-setup-top class="cv-setup-top"><span data-ui=cv-eyebrow class="cv-eyebrow">Your setup</span><span data-ui=cv-source class="cv-source" data-cv-source></span><button type="button" class="cv-link-button" data-cv-action="sync">Use toolkit settings</button></div>
        <div data-ui=cv-fields class="cv-fields"><label>Housing<select data-cv-field="housing" aria-label="Planner housing"></select><small data-cv-housing-source></small></label>
        ...
        <details data-ui=cv-assumptions class="cv-assumptions"><summary data-ui=cv-assumptions-summary>Player values and timing</summary><p>Skill inputs are base + equipment, before village bonuses. Confirmed current buildings add their skill bonuses below. The Gear tab's hypothetical add-ons are excluded. Gather yield comes from your current toolkit settings; adjust it if your in-game value differs. Planned buffs are never applied to their own construction.</p><p>Time estimates use the toolkit's housing-adjusted action durations and expected gathering yield. They assume continuous actions, current levels and no travel delay or future level-ups. Woodcutting and Construction are free for all players.</p></details>
      </div>
      <nav data-ui=cv-mobile-steps class="cv-mobile-steps" aria-label="Construction planning steps"><button type="button" data-cv-stage="0">1 · Slot</button><button type="button" data-cv-stage="1">2 · Building</button><button type="button" data-cv-stage="2">3 · Materials</button><button type="button" data-cv-stage="3">4 · Plan</button></nav>
      <div data-ui=cv-layout class="cv-layout">
        <section data-ui=cv-village class="cv-village panel" aria-label="Your current and planned village"><div data-ui=cv-section-head class="cv-section-head"><div data-ui=cv-section-content><span data-ui=cv-eyebrow class="cv-eyebrow">The village</span><h3 data-cv-village-name></h3></div><span data-ui=cv-chip class="cv-chip" data-cv-capacity></span></div>
          <div data-ui=cv-scene class="cv-scene"><div data-ui=cv-terrain class="cv-terrain" aria-hidden="true"></div><div data-ui=cv-path class="cv-path cv-path-a" aria-hidden="true"></div><div data-ui=cv-path class="cv-path cv-path-b" aria-hidden="true"></div><div data-ui=cv-house class="cv-house" data-cv-house></div><div data-ui=cv-plot class="cv-plot cv-plot-0"></div><div data-ui=cv-plot class="cv-plot cv-plot-1"></div><div data-ui=cv-plot class="cv-plot cv-plot-2"></div><div data-ui=cv-plot class="cv-plot cv-plot-3"></div><div data-ui=cv-plot class="cv-plot cv-plot-4"></div></div>
          <div data-ui=cv-legend class="cv-legend"><span data-ui=cv-legend-text>● Installed</span><span data-ui=cv-legend-text>◇ Planned</span><span data-ui=cv-legend-text>+ Available</span><span data-ui=cv-legend-text>▧ Locked</span></div>
          <p class="cv-slot-notice" data-cv-slot-notice></p><div data-ui=cv-alert class="cv-alert" data-cv-capacity-warning hidden></div>
          <details data-ui=cv-current-editor class="cv-current-editor"><summary data-ui=cv-current-editor-summary>Edit current village / choose a slot</summary><p>Enter buildings you already have installed in IdleWorlds. This is a manual record; it does not change your game or Gear plan.</p><div data-ui=cv-current-editor data-cv-current-editor></div></details>
          <div data-ui=cv-equipped-bonuses class="cv-equipped-bonuses"><div data-ui=cv-equipped-head class="cv-equipped-head"><span data-ui=cv-eyebrow class="cv-eyebrow">Equipped &amp; previewed bonuses</span><span data-ui=cv-small class="cv-small">Every building shown in your working lineup</span></div><div data-ui=cv-bonus-lineup class="cv-bonus-lineup" data-cv-bonus-lineup></div><div data-ui=cv-equipped-total class="cv-equipped-total"><span data-ui=cv-eyebrow class="cv-eyebrow">Combined active total</span><div data-ui=cv-facts class="cv-facts" data-cv-equipped-total></div></div></div>
          <div data-ui=cv-saved-plan class="cv-saved-plan"><div data-ui=cv-section-head class="cv-section-head"><h3>Village plan</h3><button type="button" class="cv-link-button" data-cv-action="clear-plan">Clear plan</button></div><div data-ui=cv-saved-plan data-cv-saved-plan></div><p class="cv-small">Saved locally. Planning never installs or destroys a building in your game.</p></div>
        </section>
        <section data-ui=cv-details class="cv-details" aria-label="Selected building and production plan">
          <div data-ui=cv-slot-intro class="cv-slot-intro panel" data-cv-stage-panel="0"><span data-ui=cv-eyebrow class="cv-eyebrow">Choose a plot</span><h3 data-cv-slot-intro></h3><p>Select a plot in your village, then choose the building you want to work toward.</p><button type="button" class="cv-primary" data-cv-next="1">Choose a building →</button></div>
          <section data-ui=cv-building-panel class="panel cv-building-panel" data-cv-stage-panel="1"><div data-ui=cv-section-head class="cv-section-head"><span data-ui=cv-eyebrow class="cv-eyebrow" data-cv-selection-label></span><span data-ui=cv-chip class="cv-chip" data-cv-selection-status></span></div><div data-ui=cv-building-hero class="cv-building-hero" data-cv-building-hero></div><div data-ui=cv-buff-lines class="cv-buff-lines" data-cv-buffs></div><div data-ui=cv-eligibility class="cv-eligibility" data-cv-eligibility></div>
            <details data-ui=cv-catalogue class="cv-catalogue"><summary data-ui=cv-catalogue-summary>Browse buildings <span data-ui=cv-catalogue-summary-text>34 Village Add-ons</span></summary><div data-ui=cv-search-controls class="cv-search-controls"><label class="cv-search-label">Find a building<input type="search" data-cv-search placeholder="Search by name or tier"></label><label>Benefit<select data-cv-filter><option value="all">All benefits</option></select></label><label class="cv-check"><input type="checkbox" data-cv-eligible>Levels met only</label></div><div data-ui=cv-catalogue-grid class="cv-catalogue-grid" data-cv-catalogue></div><p class="cv-small" data-cv-result-count></p></details>
            <div data-ui=cv-bonus-compare class="cv-bonus-compare" data-cv-bonus-compare></div><details data-ui=cv-refund class="cv-refund" hidden><summary data-ui=cv-refund-summary>Replacement refund · 25% of Assembly cost</summary><div data-ui=cv-refund data-cv-refund></div><p class="cv-small">Preview only. Refunds are not added to your inventory or gathering estimate. Fractional quantities are shown before in-game rounding. Removing the old building also removes its bonuses.</p></details>
            <div data-ui=cv-actions class="cv-actions"><button type="button" class="cv-primary" data-cv-action="add">Add to village plan</button><button type="button" class="cv-secondary cv-mobile-next" data-cv-next="2">Materials →</button></div>
          </section>
          <section data-ui=cv-material-panel class="panel cv-material-panel" data-cv-stage-panel="2"><div data-ui=cv-section-head class="cv-section-head"><div data-ui=cv-section-content><span data-ui=cv-eyebrow class="cv-eyebrow">The complete bill</span><h3>Construction materials</h3></div><label class="cv-scope-label">Calculate<select data-cv-scope><option value="selected">Selected building</option><option value="plan">Whole village plan</option></select></label></div>
            <div data-ui=cv-scope-note class="cv-scope-note" data-cv-scope-note></div><div data-ui=cv-table-wrap class="cv-table-wrap"><table data-ui=cv-table class="cv-table"><thead data-ui=cv-head><tr data-ui=cv-row><th data-ui=cv-heading>Assembly input</th><th data-ui=cv-heading>Required</th><th data-ui=cv-heading>Owned</th><th data-ui=cv-heading>Missing</th></tr></thead><tbody data-ui=cv-direct data-cv-direct></tbody></table></div>
            <details data-ui=cv-parts-details class="cv-parts-details"><summary data-ui=cv-parts-details-summary>Expand missing Parts into raw materials</summary><div data-ui=cv-parts data-cv-parts></div></details>
            <div data-ui=cv-raw-head class="cv-raw-head"><h4>Total timber & ore</h4><span data-ui=cv-small class="cv-small">Assembly + missing Parts</span></div><div data-ui=cv-table-wrap class="cv-table-wrap"><table data-ui=cv-table class="cv-table"><thead data-ui=cv-head><tr data-ui=cv-row><th data-ui=cv-heading>Raw resource</th><th data-ui=cv-heading>Required</th><th data-ui=cv-heading>Owned</th><th data-ui=cv-heading>Gather</th></tr></thead><tbody data-ui=cv-raw data-cv-raw></tbody></table></div>
            <div data-ui=cv-stock-help class="cv-stock-help"><span data-ui=cv-stock-help-text>Blank means unknown.</span><button type="button" class="cv-link-button" data-cv-action="zero-unknown">Set unknown stock to 0 for this bill</button></div><p class="cv-small">Owned Parts are deducted first. Raw stock is shared once across the complete bill.</p><button type="button" class="cv-secondary cv-mobile-next" data-cv-next="3">Production plan →</button>
          </section>
          <section data-ui=cv-production-panel class="panel cv-production-panel" data-cv-stage-panel="3"><div data-ui=cv-section-head class="cv-section-head"><div data-ui=cv-section-content><span data-ui=cv-eyebrow class="cv-eyebrow">From resources to village</span><h3>Production plan</h3></div><span data-ui=cv-chip class="cv-chip" data-cv-readiness></span></div><div data-ui=cv-alert class="cv-alert" data-cv-blockers hidden></div><ol data-ui=cv-production class="cv-production" data-cv-production></ol><div data-ui=cv-time-total class="cv-time-total"><span data-ui=cv-time-total-text>Estimated action time</span><strong data-cv-total-time>—</strong></div><p class="cv-small" data-cv-time-note></p><div data-ui=cv-actions class="cv-actions"><button type="button" class="cv-secondary" data-cv-action="copy">Copy material plan</button><button type="button" class="cv-link-button cv-mobile-next" data-cv-next="0">← Change slot</button></div></section>
        </section>
    ...
    function factRows(totals) {
      return Object.keys(totals.stats).map(function(k){return '<span data-ui=cv-fact class="cv-fact">'+esc(stat(k,totals.stats[k],true))+'</span>';}).concat(Object.keys(totals.skills).map(function(k){return '<span data-ui=cv-fact class="cv-fact">+'+fmt(totals.skills[k])+' '+esc(label(k))+'</span>';})).join('') || '<span data-ui=cv-small class="cv-small">No active bonuses in this lineup.</span>';
    }
    function renderEquippedBonuses() {
      ...
      $('[data-cv-bonus-lineup]').innerHTML=lineup.rows.map(function(row){
        ...
        return '<article data-ui=cv-bonus-card class="cv-bonus-card cv-bonus-'+row.state+(row.active?'':' cv-bonus-inactive')+(counted?'':' cv-bonus-duplicate')+'">'+
          '<div data-ui=cv-bonus-card-head class="cv-bonus-card-head">'+sprite(row.tier)+'<div data-ui=cv-bonus-card-content><strong>'+esc(b.name)+'</strong><span data-ui=cv-bonus-card-text>Slot '+(row.slot+1)+' · '+esc(stateLabel)+'</span></div></div>'+
          '<div data-ui=cv-bonus-card-lines class="cv-bonus-card-lines"><span data-ui=cv-bonus-card-lines-text><small>Primary</small>'+esc(stat(b.buffs.primary.stat,b.buffs.primary.value,true))+'</span><span data-ui=cv-bonus-card-lines-text><small>Secondary</small>'+b.buffs.secondary.map(function(v){return esc(stat(v.stat,v.value,true));}).join(' · ')+'</span><span data-ui=cv-bonus-card-lines-text><small>Skill</small>+'+fmt(b.buffs.skill.amount)+' '+esc(label(b.buffs.skill.skill))+'</span></div></article>';
    ...
    function renderVillage() {
      ...
      $('[data-cv-house]').innerHTML = housingArt(setup.housing)+'<strong>'+esc(o.houseName(setup.housing))+'</strong><span data-ui=render-village-text>Housing tier '+setup.housing+'</span>';
      for(var i=0;i<5;i++) {
        ...
        var art=tier?sprite(tier):'<span data-ui=cv-empty-plot class="cv-empty-plot">'+o.houseArt(0)+'</span>';
        $('.cv-plot-'+i).innerHTML='<button type="button" class="cv-plot-button '+(planned&&planned!==current?'cv-planned ':'')+(locked?'cv-locked ':'')+'" data-cv-slot="'+i+'" aria-pressed="'+(state.selectedSlot===i)+'" aria-label="Slot '+(i+1)+' · '+esc(status)+(tier?' · '+esc(building(tier).name):'')+'">'+art+'<span data-ui=cv-plot-name class="cv-plot-name">'+esc(tier?building(tier).name:'Slot '+(i+1))+'</span><span data-ui=cv-plot-status class="cv-plot-status">'+(locked?'Needs '+esc(o.houseName(i+1)):status)+'</span></button>';
      }
      ...
      $('[data-cv-current-editor]').innerHTML=state.installed.map(function(t,i){return '<div data-ui=cv-current-row class="cv-current-row"><button type="button" class="cv-secondary" data-cv-slot="'+i+'">Slot '+(i+1)+'</button><label><span data-ui=cv-sr-only class="cv-sr-only">Installed building in slot '+(i+1)+'</span><select data-cv-installed="'+i+'"><option value="">Empty</option>'+Array.from({length:34},function(_,j){var tier=j+1,used=state.installed.some(function(v,s){return s!==i&&v===tier;});return '<option value="'+tier+'"'+(t===tier?' selected':'')+(used?' disabled':'')+'>T'+tier+' · '+esc(building(tier).name)+'</option>';}).join('')+'</select></label>'+(i>=village.capacity?'<span data-ui=cv-small class="cv-small">Inactive</span>':'')+'</div>';}).join('');
      renderEquippedBonuses();
      var plans=state.planned.map(function(t,i){if(t==null)return '';return '<div data-ui=cv-plan-row class="cv-plan-row"><button type="button" class="cv-plan-select" data-cv-slot="'+i+'">'+sprite(t)+'<span data-ui=cv-plan-text>Slot '+(i+1)+' · '+esc(building(t).name)+'<small>'+(i>=village.capacity?'Housing locked':state.installed[i]===t?'Already installed':state.installed[i]?'Replacement plan':'New add-on')+'</small></span></button><button type="button" class="cv-link-button" data-cv-remove="'+i+'" aria-label="Remove plan for slot '+(i+1)+'">Remove</button></div>';}).join('');
    ...
    function comparisonRows(current,next) {
      var html='';['stats','skills'].forEach(function(group){Object.keys(Object.assign({},current[group],next[group])).forEach(function(k){var a=current[group][k]||0,b=next[group][k]||0;if(a===b)return;html+='<div data-ui=cv-delta class="cv-delta"><span data-ui=cv-delta-text>'+esc(label(k))+'</span><span data-ui=cv-delta-text>'+fmt(a)+' → '+fmt(b)+'</span><strong class="'+(b>=a?'cv-gain':'cv-loss')+'">'+esc(stat(k,b-a,false))+'</strong></div>';});});return html;
    }
    function renderBuilding() {
      ...
      $('[data-cv-building-hero]').innerHTML='<div data-ui=cv-hero-art class="cv-hero-art">'+sprite(t)+'</div><div data-ui=render-building-content><span data-ui=cv-eyebrow class="cv-eyebrow">Village Add-on · Tier '+t+'</span><h3>'+esc(b.name)+'</h3><span data-ui=cv-small class="cv-small">Permanent passive benefits while installed</span></div>';
      $('[data-cv-buffs]').innerHTML='<div data-ui=render-building-content><span data-ui=render-building-text>Primary</span><strong>'+esc(stat(b.buffs.primary.stat,b.buffs.primary.value,true))+'</strong></div><div data-ui=render-building-content><span data-ui=render-building-text>Secondary</span><strong>'+b.buffs.secondary.map(function(v){return esc(stat(v.stat,v.value,true));}).join(' · ')+'</strong></div><div data-ui=render-building-content><span data-ui=render-building-text>Skill level</span><strong>+'+b.buffs.skill.amount+' '+esc(label(b.buffs.skill.skill))+'</strong></div>';
    ...
    function rowHtml(r,kind) {
      return '<tr data-ui=cv-row data-cv-row="'+r.id+'"><td data-ui=cv-cell>'+item(r)+'</td><td data-ui=cv-required data-cv-required="'+r.id+'">'+(r.provisional?'Up to ':'')+fmt(r.required)+'</td><td data-ui=cv-cell><input type="number" min="0" max="9007199254740991" step="1" inputmode="numeric" data-cv-stock="'+r.id+'" aria-label="Owned '+esc(r.name)+(kind==='raw'?' raw stock':'')+'" value="'+(state.stock[r.id]==null?'':state.stock[r.id])+'" placeholder="?"></td><td data-ui=cv-missing data-cv-missing="'+r.id+'">'+(r.missing==null?'?':fmt(r.missing))+'</td></tr>';
    }
    ...
    function refreshResults(preserve) {
      ...
      $('[data-cv-parts]').innerHTML=r.partsRows.map(function(p){return '<div data-ui=cv-parts-row class="cv-parts-row"><strong>'+(p.missing==null?'Up to '+fmt(p.upperBound):fmt(p.missing))+' '+esc(p.name)+'</strong><span data-ui=cv-parts-text>'+fmt(p.upperBound*2*p.tier)+' '+esc(o.adapter.woodName(p.tier))+' + '+fmt(p.upperBound*p.tier)+' '+esc(o.adapter.oreName(p.tier))+'</span></div>';}).join('')||'<p class="cv-small">No Parts needed.</p>';
      ...
      $('[data-cv-blockers]').hidden=!warnings.length;$('[data-cv-blockers]').innerHTML=warnings.map(function(w){return '<div data-ui=refresh-results-content>'+esc(w)+'</div>';}).join('');
      ...
      $('[data-cv-production]').innerHTML=steps.map(function(step,i){return '<li data-ui=cv-production-step><span data-ui=cv-step-number class="cv-step-number">'+(i+1)+'</span><div data-ui=cv-production-step-content><strong>'+step[0]+'</strong><p>'+step[1]+'</p></div><span data-ui=cv-step-time class="cv-step-time">'+duration(step[2])+'</span></li>';}).join('');
